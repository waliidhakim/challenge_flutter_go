...
class ParticipationService {
  static Future<Map<DateTime, List<Event>>> fetchParticipations(String userId) async {
    if (userId.isEmpty) return {};

    final apiUrl = AppSettings().apiUrl;

    final response = await http.get(
      Uri.parse('$apiUrl/users/$userId/group_chat_activity_participations'),
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      for (var participation in data) {
        final DateTime date = DateTime.parse(participation['participation_date']);
